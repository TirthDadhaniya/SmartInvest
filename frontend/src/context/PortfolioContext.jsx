...
export const PortfolioProvider = ({ children }) => {
  ...
  const [portfolioData, setPortfolioData] = useState(emptyPortfolioState);

  const fetchPortfolio = useCallback(async () => {
    if (!user) {
      setPortfolioData(emptyPortfolioState);
      return;
    }
    ...
    try {
      ...
      const totalProfitLoss = parseFloat(financials.totalProfitLoss) || unrealizedProfit;
      ...
      const annualExpenseCost = drainData?.annualCost || totalInvested * (weightedExpRatio / 100);
      const tenYearOpportunityCost = drainData?.tenYearCost || annualExpenseCost * 10 * 1.5;
      ...
      const mappedData = {
        ...
        assetAllocation: {
          equity:
            parseFloat(
              payload.health?.metrics?.allocationChart?.find(i => i.name.toLowerCase() === 'equity')
                ?.percentage
            ) || 0,
          debt:
            parseFloat(
              payload.health?.metrics?.allocationChart?.find(i => i.name.toLowerCase() === 'debt')
                ?.percentage
            ) || 0,
          hybrid:
            parseFloat(
              payload.health?.metrics?.allocationChart?.find(i => i.name.toLowerCase() === 'hybrid')
                ?.percentage
            ) || 0,
        },
        ...
        expenseInfo: {
          ...
          annualExpenseCost,
          tenYearOpportunityCost,
        },
      };

      const notifications = mappedData.tips.map((tip, idx) => ({
        id: `tip-${Date.now()}-${idx}`,
        text: tip,
        isRead: false,
